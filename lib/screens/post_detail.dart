...
class _PostDetailScreenState extends State<PostDetailScreen> {
  ...
  Future<void> _fetchComments() async {
    final uri = Uri.parse(
      '${dotenv.env['API_URL']}/api/comments?post_id=${widget.post.id}',
    );
    ...
    if (res.statusCode == 200) {
      ...
      debugPrint('댓글 가져오기 실패: ${res.statusCode}');
    }
  }
  ...
  Widget _buildCommentItem(Comment comment) {
    final currentUser = FirebaseAuth.instance.currentUser;
    final isMe = currentUser?.uid == comment.userUid;
    final createdAt = comment.createdAt.toLocal();
    final dateText = "${createdAt.year}.${_two(createdAt.month)}.${_two(createdAt.day)} "
      ...
      children: [
        ListTile(
          contentPadding: EdgeInsets.only(
            left: comment.parentId != null ? 48 : 0,
            right: 4,
          ),
